# gctln_w_uniq_edges.py  — updated to consume NetworkSpec
import torch
...
class GCTLNPerEdge(nn.Module):
    ...
    def __init__(self, spec: NetworkSpec):
        ...
        # Per-neuron tonic drive
        self.theta = nn.Parameter(torch.ones(self.n) * 0.1)
        ...
        # so training starts near a known working point
    ...
    def to_numpy_W(self) -> np.ndarray:
        """
        Export the learned weight matrix as a numpy array.
        Can be plugged directly into source.py's simulate().
        """
        with torch.no_grad():
    ...
    def simulate_numpy(self, theta: float, T: float,
                        n_steps: int, x0: np.ndarray):
        """
        Run a numpy simulation using the learned weights.
        Returns (t, sol) compatible with all source.py plotting functions.
        """
        W_np = self.to_numpy_W()
        return source.simulate(W_np, theta, T, n_steps, x0)

    def visualize_learned(self, theta: float = 0.1,
                           T: float = 500, n_steps: int = 15000,
        ...
        t, sol = self.simulate_numpy(theta, T, n_steps, x0)
    ...
    def plot_graph(self, show_weights=True, show_theta=True,
               title="", **kwargs):
        """
        Visualize the network graph with learned weights and theta values.
        """
...
def train(model: GCTLNPerEdge,
    ...
    for epoch in range(n_epochs):
        ...
        loss        = recon_loss + penalty_weight * penalty
        loss.backward()
...
if __name__ == "__main__":
    ...
    targets = torch.stack([
        ...
        torch.sin(t_span),                        # CPG-B mirrors CPG-A
        torch.sin(t_span + phase_offset),
        torch.sin(t_span + 2 * phase_offset),
    ], dim=1)
    ...
    model = GCTLNPerEdge(spec)
    loss_h = train(model, targets, t_span, x0_pt)
    model.plot_graph(title="Learned gCTLN weights")

    # For large networks, hide non-edges and weights to reduce clutter
    # plot_network_graph(spec, W=model.to_numpy_W(),
    #                    theta=model.theta.detach().numpy(),
    #                    show_weights=False,   # hide edge labels
    #                    show_theta=False,     # just neuron indices
    #                    figsize=(14, 12))
...
model.visualize_learned(theta=0.1, T=500, n_steps=15000)

# ── 8. Compare prediction vs target ──────────────────────────────────
n_out = spec.n_total

# Build axis labels dynamically from spec
# Each neuron gets a label: "MotifLabel / n{local_idx}"
axis_labels = []
...
# Scale figure height to number of outputs
...
# Color each subplot by its motif
